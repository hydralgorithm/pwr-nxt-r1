...
Requires Python 3.10+ with `pandas numpy scikit-learn lightgbm catboost scipy openpyxl`
(install via `pip install -r requirements.txt`). No manual record edits; all thresholds derive
from the data at run time; outputs verified identical on re-run.
