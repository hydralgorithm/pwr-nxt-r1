...
because any single value looks unusual: a logistic regression on raw sensor
readings achieves AUC 0.502 (chance level) for validity, while relationship
features raise it above 0.999. This is the organizers' doctrine — "an
unusual value is not necessarily an invalid value" — made quantitative, and
it is why the design never uses univariate outlier thresholds.
...
miscalibration indistinguishable from honest noise) with zero false flags.
...
deterministic in the inputs: `Ref ~ I + I² + Ambient + Voltage + Duration`
(Joule heating) reaches R² = 0.980 using **no sensors at all**. Sensors are
exactly the corrupted channels in Invalid rows, so the model deliberately
uses operating inputs only — every prediction (Valid or Invalid) comes from
one corruption-immune mechanism. The deployed model is a
**physics-informed hybrid**: the OLS physics term carries the
extrapolatable physics, and a 5-seed bagged CatBoost (depth 4) learns only
the bounded residual correction. CV on Valid rows: **RMSE 0.41 °C, MAE
0.27, R² 0.9985** — better than pure CatBoost in range (0.52) and far
safer beyond it. The out-of-fold residual correlates at most |r| = 0.06
with any input: the score sits at the data's irreducible noise floor.
...
and rejected — no gain, added corruption exposure.

## 3. Robustness stress tests (hidden-dataset insurance)

- **Extrapolation** (train without the top quintile of each operating
  axis): the hybrid beats pure CatBoost on current (2.7 vs 9.4 — tree
  models plateau at the training boundary), voltage (0.34 vs 0.76),
  ambient (2.47 vs 3.51) and duration (0.53 vs 0.65).
- **Label noise:** with 5% of training labels randomly flipped, accuracy
  degrades gracefully to 0.946.
- **Poisoned labels:** duplicate groups' Reference values disagree by
  ~12 °C — why Invalid rows are excluded from regression training.
- **Self-monitoring:** the pipeline emits a 90% prediction error band
  (±0.53 °C) and an automatic input-drift check (Kolmogorov–Smirnov per
  operating input) in summary.json, so a run on shifted data announces it
...
- **Ensemble decision threshold 0.5** on the mean probability
  (CV-optimal); the 13.4% class imbalance is handled by class weights
  inside each member.
...
that mutual-agreement manifold. Genuine regime shifts (high current) move
all three sensors together — residuals stay small, so a genuinely hot test
is never flagged; a faulty sensor breaks the correlation and is caught.
...
1. The Reference_Parameter of Invalid rows is untrustworthy (duplicate
   ...
   on Valid rows only.
2. Hidden-dataset conditions stay within (or near) the training operating
   envelope; the hybrid's physics base covers near-range extension.
...
2. **Score:** every new record is scored in milliseconds — per batch or
   streaming.
...
6. **Audit:** summary.json written on every run gives machine-readable
   counts, extremes, attention IDs and validation metrics for dashboards.
