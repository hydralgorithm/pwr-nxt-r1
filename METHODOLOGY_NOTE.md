...
the top current quintile (92–110 A), pure CatBoost's RMSE on that band is 9.4
while the hybrid holds 2.7. Since the hidden second dataset may stress-test beyond nominal
...
- **Task 1 error analysis (out-of-fold):** exactly **1 missed Invalid of 134**
  (TRN-0766 — a borderline S3 miscalibration whose residual sits at the 12th
  percentile of the Valid residual distribution, i.e. indistinguishable from
  honest noise) and **0 false-flagged Valid rows**. Catching that one row
  would require thresholds loose enough to flag many Valid rows; rejected as
  a bad trade for unseen data.
- **Task 2 extrapolation** (withholding the top quintile of each axis; all
  numbers regenerated by `src/stress_tests.py` → `results/stress_tests.txt`):
  pure CatBoost predicts the withheld 92–110 A band at RMSE 9.4 (tree
  plateau); the deployed hybrid holds 2.7. High-voltage band (27.9–32.0 kV):
  hybrid 0.34 vs 0.76. High-ambient band (40.5–55 °C): 2.47 vs 3.51.
  Long-duration band (47.5–60 min): 0.53 vs 0.65. The hybrid wins on every
  stress axis measured, so the choice is not an artifact of any single test.
...
- `src/stress_tests.py` — regenerates every stress-test number cited in
  section 2b into `results/stress_tests.txt` (extrapolation on four axes,
  label-noise sensitivity, error analysis, noise-floor diagnostic).
